...
// To focus on the first item when the dropdown is opened
acctEl.addEventListener("click", function () {
  const isExpanded = acctEl.attributes["aria-expanded"].value === "true";

  const allAccountDropdownItems = dropdownEl.querySelectorAll(
    '[role = "menuitem"]'
  );

  if (isExpanded) {
    acctEl.ariaExpanded = "false";
    acctEl.focus();
  } else {
    acctEl.ariaExpanded = "true";
    allAccountDropdownItems.item(0).focus();
  }
});

// For closing of the menu upon pressing the esc key
...
// FOR THE ALERT DROPDOWN
alertEl.addEventListener("click", function () {
  const isExpandedAlert = alertEl.attributes["aria-expanded"].value === "true";
  if (isExpandedAlert) {
    alertEl.ariaExpanded = "false";
  } else {
    alertEl.ariaExpanded = "true";
  }
});
...
setupGuideTogglerEl.addEventListener("click", () => {
  ...
  if (setupGuideBtmHeight === "320px") {
    ...
    setupGuideBottomEl.style.height = "320px";
